...
public class ChildHelper {
     ...
     * @param type     View type, can be {@link DRecyclerView#INVALID_TYPE}.
    ...
    View findHiddenNonRemovedView(int position, int type) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            DRecyclerView.ViewHolder holder = mCallback.getChildViewHolder(view);
            if (holder.getLayoutPosition() == position && !holder.isInvalid() && !holder.isRemoved()
                    && (type == DRecyclerView.INVALID_TYPE || holder.getItemViewType() == type)) {
                return view;
            }
        }
        return null;
    ...
    static interface Callback {
        ...
        DRecyclerView.ViewHolder getChildViewHolder(View view);
