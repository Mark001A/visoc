...
import com.dovar.common.vsview.DRecyclerView.ItemAnimator.ItemHolderInfo;

import static com.dovar.common.vsview.ViewInfoStore.InfoRecord.FLAG_APPEAR;
import static com.dovar.common.vsview.ViewInfoStore.InfoRecord.FLAG_APPEAR_AND_DISAPPEAR;
import static com.dovar.common.vsview.ViewInfoStore.InfoRecord.FLAG_APPEAR_PRE_AND_POST;
import static com.dovar.common.vsview.ViewInfoStore.InfoRecord.FLAG_DISAPPEARED;
import static com.dovar.common.vsview.ViewInfoStore.InfoRecord.FLAG_POST;
import static com.dovar.common.vsview.ViewInfoStore.InfoRecord.FLAG_PRE;
import static com.dovar.common.vsview.ViewInfoStore.InfoRecord.FLAG_PRE_AND_POST;
...
public class ViewInfoStore {
     ...
     */
    @VisibleForTesting
    final ArrayMap<DRecyclerView.ViewHolder, InfoRecord> mLayoutHolderMap = new ArrayMap<>();

    @VisibleForTesting
    final LongSparseArray<DRecyclerView.ViewHolder> mOldChangedHolders = new LongSparseArray<>();
     ...
     */
    void addToPreLayout(DRecyclerView.ViewHolder holder, ItemHolderInfo info) {
     ...
     */
    @Nullable
    DRecyclerView.ItemAnimator.ItemHolderInfo popFromPreLayout(RecyclerView.ViewHolder vh) {
     ...
     */
    @Nullable
    ItemHolderInfo popFromPostLayout(RecyclerView.ViewHolder vh) {
        return popFromLayoutStep(vh, FLAG_POST);
    }

    private ItemHolderInfo popFromLayoutStep(RecyclerView.ViewHolder vh, int flag) {
     ...
     */
    void addToOldChangeHolders(long key, DRecyclerView.ViewHolder holder) {
     ...
     * @param info   The information to save
     */
    void addToAppearedInPreLayoutHolders(DRecyclerView.ViewHolder holder, ItemHolderInfo info) {
        InfoRecord record = mLayoutHolderMap.get(holder);
        if (record == null) {
            record = InfoRecord.obtain();
            mLayoutHolderMap.put(holder, record);
     ...
     */
    DRecyclerView.ViewHolder getFromOldChangeHolders(long key) {
     ...
     * @param info   The information to save
     */
    void addToPostLayout(DRecyclerView.ViewHolder holder, ItemHolderInfo info) {
        InfoRecord record = mLayoutHolderMap.get(holder);
        if (record == null) {
            record = InfoRecord.obtain();
            mLayoutHolderMap.put(holder, record);
     ...
     */
    void addToDisappearedInLayout(DRecyclerView.ViewHolder holder) {
        InfoRecord record = mLayoutHolderMap.get(holder);
        if (record == null) {
            record = InfoRecord.obtain();
            mLayoutHolderMap.put(holder, record);
     ...
     */
    void removeFromDisappearedInLayout(DRecyclerView.ViewHolder holder) {
        InfoRecord record = mLayoutHolderMap.get(holder);
        if (record == null) {
            return;
        }
    ...
    void process(ProcessCallback callback) {
        for (int index = mLayoutHolderMap.size() - 1; index >= 0; index--) {
            final DRecyclerView.ViewHolder viewHolder = mLayoutHolderMap.keyAt(index);
     ...
     */
    void removeViewHolder(DRecyclerView.ViewHolder holder) {
    ...
    public void onViewDetached(DRecyclerView.ViewHolder viewHolder) {
        removeFromDisappearedInLayout(viewHolder);
    }

    interface ProcessCallback {
        void processDisappeared(DRecyclerView.ViewHolder viewHolder, @NonNull ItemHolderInfo preInfo,
                                @Nullable ItemHolderInfo postInfo);

        void processAppeared(DRecyclerView.ViewHolder viewHolder, @Nullable ItemHolderInfo preInfo,
                             ItemHolderInfo postInfo);

        void processPersistent(DRecyclerView.ViewHolder viewHolder, @NonNull ItemHolderInfo preInfo,
                               @NonNull ItemHolderInfo postInfo);

        void unused(DRecyclerView.ViewHolder holder);
    }
