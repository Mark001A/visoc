...
import static com.dovar.common.vsview.DRecyclerView.NO_POSITION;

/**
 * Created by heweizong on 2018/5/8.
 */

public class LinearLayoutManager extends DRecyclerView.LayoutManager implements ItemTouchHelper.ViewDropHandler, DRecyclerView.SmoothScroller.ScrollVectorProvider {
    ...
    /**
     * Constructor used when layout manager is set in XML by DRecyclerView attribute
    ...
    public LinearLayoutManager(Context context, AttributeSet attrs, int defStyleAttr,
                               int defStyleRes) {
        DRecyclerView.LayoutManager.Properties properties = getProperties(context, attrs, defStyleAttr, defStyleRes);
     ...
     */
    @Override
    public DRecyclerView.LayoutParams generateDefaultLayoutParams() {
        return new DRecyclerView.LayoutParams(ViewGroup.LayoutParams.WRAP_CONTENT,
     ...
     * DRecyclerView.
     * <p>
     * If you are using a {@link DRecyclerView.RecycledViewPool}, it might be a good idea to set
     * this flag to <code>true</code> so that views will be available to other RecyclerViews
     * immediately.
     * <p>
     * Note that, setting this flag will result in a performance drop if DRecyclerView
     * is restored.
     *
     ...
     * When set to true, If {@link DRecyclerView} is LTR, than it will
     * layout from RTL, if {@link DRecyclerView}} is RTL, it will layout
     * from LTR.
     * <p>
    ...
    private void layoutForPredictiveAnimations(DRecyclerView.Recycler recycler,
        ...
        final List<DRecyclerView.ViewHolder> scrapList = recycler.getScrapList();
        final int scrapSize = scrapList.size();
        final int firstChildPos = getPosition(getChildAt(0));
        for (int i = 0; i < scrapSize; i++) {
            DRecyclerView.ViewHolder scrap = scrapList.get(i);
            if (scrap.isRemoved()) {
                continue;
            }
    ...
    /**
     * <p>Scroll the DRecyclerView to make the position visible.</p>
     * <p>
     * <p>DRecyclerView will scroll the minimum amount that is necessary to make the
     ...
     * <code>item[10]</code>'s bottom is 20 pixels above the DRecyclerView's bottom.
     * <p>
     ...
     *                 start edge of the DRecyclerView.
     * @see #setReverseLayout(boolean)
     ...
     */
    private void recycleChildren(DRecyclerView.Recycler recycler, int startIndex, int endIndex) {
     ...
     *
     * @param recycler Recycler instance of {@link DRecyclerView}
     * @param dt       This can be used to add additional padding to the visible area. This is used
     *                 to detect children that will go out of bounds after scrolling, without
     *                 actually moving them.
     */
    private void recycleViewsFromStart(DRecyclerView.Recycler recycler, int dt) {
        if (dt < 0) {
            if (DEBUG) {
     ...
     *                 actually moving them.
     */
    private void recycleViewsFromEnd(DRecyclerView.Recycler recycler, int dt) {
        final int childCount = getChildCount();
        if (dt < 0) {
            if (DEBUG) {
     ...
     *
     * @param recycler    Current recycler that is attached to DRecyclerView
     * @param layoutState Current layout state. Right now, this object does not change but
     *                    we may consider moving it out of this view so passing around as a
     *                    parameter for now, rather than accessing {@link #mLayoutState}
     * @see #recycleViewsFromStart(DRecyclerView.Recycler, int)
     * @see #recycleViewsFromEnd(DRecyclerView.Recycler, int)
     * @see android.support.v7.widget.LinearLayoutManager.LayoutState#mLayoutDirection
     */
    private void recycleByLayoutState(DRecyclerView.Recycler recycler, LayoutState layoutState) {
     ...
     * @param state           Context passed by the DRecyclerView to control scroll steps.
    ...
    void layoutChunk(DRecyclerView.Recycler recycler, State state,
        ...
        DRecyclerView.LayoutParams params = (DRecyclerView.LayoutParams) view.getLayoutParams();
    ...
    View findReferenceChild(DRecyclerView.Recycler recycler, State state,
        ...
        for (int i = start; i != end; i += diff) {
            ...
            if (position >= 0 && position < itemCount) {
                if (((DRecyclerView.LayoutParams) view.getLayoutParams()).isItemRemoved()) {
     ...
     * not in the layout.
     * <p>
     * If DRecyclerView has item decorators, they will be considered in calculations as well.
     * <p>
     * LayoutManager may pre-cache some views that are not necessarily visible. Those views
     * are ignored in this method.
     *
     * @return The adapter position of the first visible item or {@link DRecyclerView#NO_POSITION} if
     ...
     * {@link DRecyclerView#NO_POSITION} if there aren't any visible items.
     ...
     * are ignored in this method.
     *
     * @return The adapter position of the last visible view or {@link DRecyclerView#NO_POSITION} if
     * there aren't any visible items.
     * @see #findLastCompletelyVisibleItemPosition()
     * @see #findFirstVisibleItemPosition()
     */
    ...
    static class LayoutState {
         ...
         */
        List<DRecyclerView.ViewHolder> mScrapList = null;
         ...
         */
        View next(DRecyclerView.Recycler recycler) {
            if (mScrapList != null) {
        ...
        public void assignPositionFromScrapList(View ignore) {
            ...
            if (closest == null) {
                ...
                mCurrentPosition = ((DRecyclerView.LayoutParams) closest.getLayoutParams())
    ...
    class AnchorInfo {
        ...
        /**
         * assigns anchor coordinate from the DRecyclerView's padding depending on current
         * layoutFromEnd value
         */
